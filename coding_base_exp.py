...
if __name__ == "__main__":
    ...
    for tmp_round in range(num_comm_rounds):
        ...
        for tmp_key in weight_dict.keys():
            for array_id, array in weight_dict[tmp_key].items():
                ...
                if loss_rate > select_random:
                    weight_dict[tmp_key][array_id] = np.zeros_like(array)
        ...
        for i in range(len(local_model)):
            ...
            for array_id, array in enumerate(tmp_weight_list):
                select_random = random.random()
                if loss_rate > select_random:
                    tmp_weight_list[array_id] = np.zeros_like(array)
    ...
    with open(f"result/fl_base_loss_{options.lose}_E_{options.epoch}_B_{options.batch}.pkl", "wb") as f:
